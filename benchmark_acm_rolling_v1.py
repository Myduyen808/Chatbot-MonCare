"""
Benchmark 5 chiến lược quản lý ngữ cảnh cho MomCare — Rolling Summary
=================================================================
...
- ACM được mô phỏng theo Rolling Summary có trạng thái, cập nhật theo từng
  cặp Human + AI thay vì tóm tắt lại toàn bộ lịch sử ở mỗi lượt.

Chạy thử một kịch bản:
    python benchmark_acm_rolling_v1.py --scenario vitamin_d

Chạy cả ba kịch bản:
    python benchmark_acm_rolling_v1.py --all
"""
...
# Rolling ACM luôn giữ nguyên 2 tin nhắn gần nhất.
HISTORY_KEEP_RECENT = 2

# Chỉ cập nhật summary khi có ít nhất 2 tin nhắn cũ chưa xử lý
# và tổng độ dài của phần đó đạt ngưỡng tối thiểu.
ROLLING_SUMMARY_MIN_MESSAGES = 2
ROLLING_SUMMARY_TRIGGER_CHARS = 250

SLEEP_BETWEEN_METHODS = 3
OUTPUT_PREFIX = "acm_rolling_comparison_v1"
...
def build_acm(
    ...
    """
    Mô phỏng Rolling Summary theo đúng luồng nhiều lượt:

    - Lịch sử được duyệt tăng dần theo từng cặp Human + AI.
    - Hai tin nhắn gần nhất luôn được giữ nguyên.
    - Chỉ phần lịch sử cũ chưa từng được tóm tắt mới được đưa vào
      update_rolling_summary().
    - Không tóm tắt lại toàn bộ lịch sử từ đầu ở mỗi lượt.
    """
    if not history:
        return [], "empty"

    raw_dicts = to_dicts(history)

    rolling_summary = ""
    summarized_count = 0
    mode = "keep_all"

    # Mô phỏng trạng thái sau từng lượt hoàn chỉnh.
    # Ví dụ: 2, 4, 6, ... tin nhắn.
    for current_size in range(2, len(raw_dicts) + 1, 2):
        current_history = raw_dicts[:current_size]

        split_index = max(
            0,
            len(current_history) - HISTORY_KEEP_RECENT,
        )
        ...
        pending_messages = current_history[
        ...
        if enough_messages and enough_chars:
            ...
            candidate_summary = update_rolling_summary(
                previous_summary=rolling_summary,
    ...
    # Tạo ngữ cảnh cuối cùng dùng cho Query Rewriting.
    ...
    # Giữ tất cả tin chưa được đưa vào summary.
    remaining = history[summarized_count:]
    processed.extend(remaining)

    if not rolling_summary:
        mode = "keep_all"
    elif mode not in {
        "rolling_summary_create",
...
SCENARIOS: dict[str, Scenario] = {
    "vitamin_d": Scenario(
        ...
        required_groups=[
            ["vitamin d"],
            ["6 tháng", "trẻ 6 tháng"],
            ["nguồn bổ sung", "bổ sung từ", "nguồn nào", "nguồn thực phẩm", "nguồn cung cấp"],
        ],
    ...
    "oral_care": Scenario(
        ...
        required_groups=[
            ["vệ sinh răng miệng", "vệ sinh miệng", "làm sạch lợi", "chăm sóc vùng miệng", "dụng cụ vệ sinh"],
            ["6 tháng", "trẻ 6 tháng"],
...
def run_method(
    ...
    rewrite_content_ok, matched, missing = check_rewrite(
        rewritten,
        scenario.required_groups,
    )

    # Một trường hợp chỉ được tính là đạt khi:
    # - nội dung câu viết lại giữ đủ ngữ cảnh;
    # - intent vẫn được điều hướng đúng sang RAG.
    context_ok = (
        rewrite_content_ok
        and intent == "RAG"
    )
    ...
    return {
        ...
        "intent": intent,
        "rewrite_content_ok": rewrite_content_ok,
...
def main() -> None:
    ...
    parser.add_argument(
        "--all",
        action="store_true",
        help="Chạy cả ba kịch bản với Rolling Summary.",
    )
