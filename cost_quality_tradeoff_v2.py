...
Cách chạy:
    python cost_quality_tradeoff_v2.py

Input v2:
    - telemetry_all_seeds_v2.csv
    - telemetry_raw_log_seed*_v2.csv
    - telemetry_raw_log_v2.csv

Output v2:
    - cost_quality_summary_v2.csv
    - cost_quality_summary_v2.xlsx
    - Pareto_Frontier_Chart_v2.png
"""
...
def load_telemetry():
    """
    Chỉ đọc dữ liệu phiên bản v2 để không trộn với kết quả cũ.
    Thứ tự ưu tiên:
      1. telemetry_all_seeds_v2.csv
      2. telemetry_raw_log_seed*_v2.csv
      3. telemetry_raw_log_v2.csv
    """
    primary = "telemetry_all_seeds_v2.csv"
    if os.path.exists(primary):
        df = pd.read_csv(primary)
        print(f"✅ Đã nạp '{primary}' ({len(df)} dòng).")
        return df

    seed_files = sorted(glob.glob("telemetry_raw_log_seed*_v2.csv"))
    if seed_files:
        frames = []
        ...
        print(
            f"✅ Đã gộp {len(seed_files)} file seed v2 "
            f"thành {len(df)} dòng."
        )
        return df

    single = "telemetry_raw_log_v2.csv"
    if os.path.exists(single):
        df = pd.read_csv(single)
        print(f"✅ Đã nạp '{single}' ({len(df)} dòng).")
        print("⚠️ Đây là dữ liệu một lần chạy, không phải multi-seed.")
        return df

    raise FileNotFoundError(
        "❌ Không tìm thấy dữ liệu v2. Hãy tạo một trong các file:\n"
        "   telemetry_all_seeds_v2.csv\n"
        "   telemetry_raw_log_seed*_v2.csv\n"
        "   telemetry_raw_log_v2.csv\n"
        "File dữ liệu cũ sẽ không được đọc."
    )
...
def validate_input_columns(df):
    """Kiểm tra các cột bắt buộc của file telemetry."""
    required = {"config", "faithfulness", "latency_ms"}
    ...
    has_old_tokens = "tokens" in df.columns

    if not (has_new_tokens or has_old_tokens):
        raise ValueError(
            "Cần có prompt_tokens và completion_tokens, "
            "hoặc cột tokens của định dạng cũ."
        )
...
def main():
    ...
    required_non_null = ["config", "faithfulness", "latency_ms"]
    if {"prompt_tokens", "completion_tokens"}.issubset(df.columns):
        required_non_null += ["prompt_tokens", "completion_tokens"]
    else:
        required_non_null += ["tokens"]

    before = len(df)
    df = df.dropna(subset=required_non_null).copy()
    if len(df) < before:
        print(f"⚠️ Đã loại {before - len(df)} dòng dữ liệu không hợp lệ.")

    if df.empty:
        print("❌ Không còn dữ liệu hợp lệ để tính toán.")
        sys.exit(1)

    df["config"] = df["config"].astype(str).str.strip().str.upper()
    ...
    if summary.empty:
        print("❌ Không có dữ liệu cho các cấu hình A, B, C hoặc D.")
        sys.exit(1)
    ...
    fig.tight_layout()
    fig.savefig("Pareto_Frontier_Chart_v2.png", dpi=150)
    print("\n✅ Đã lưu biểu đồ 'Pareto_Frontier_Chart_v2.png'.")
    ...
    print("✅ Đã lưu bảng tổng hợp vào 'cost_quality_summary_v2.csv'.")
    ...
    try:
        ...
        print("✅ Đã lưu bảng tổng hợp vào 'cost_quality_summary_v2.xlsx'.")
    except ImportError:
        print(
            "⚠️ Thiếu openpyxl nên chưa tạo được file Excel; "
            "file CSV vẫn đã được lưu."
        )
